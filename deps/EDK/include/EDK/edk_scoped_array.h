 ...
 * Scoped_array, original code from Boost library.
 *
...
#ifndef __EDK_SCOPED_ARRAY_H__
#define __EDK_SCOPED_ARRAY_H__
...
template<class T, class T_index = unsigned int>
class scoped_array {  // noncopyable
 public:
  ...
  T* alloc(size_t num_elements) {
    internal_reset(new T[num_elements], (T_index) num_elements);
    return px_;
  }
  ...
  void copy(const T *begin, const T *end) {
    ...
    internal_reset(reinterpret_cast<T*>(new T[elements]), 
                   (T_index) elements);
