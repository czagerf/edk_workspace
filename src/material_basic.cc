...
#define GLSL(x) "#version 330\n"#x
    ...
    MaterialBasic::~MaterialBasic() {}
    MaterialBasic::MaterialBasic(const MaterialBasic& other) { program_ = other.program_; }
    ...
    const bool MaterialBasic::enable(const EDK::MaterialSettings* mat) const {
        //Enable the material...
        //... and use the uniforms located in the material settings!
        program_->use();
        typedef MaterialBasicSettings MBS;
    ...
    EDK::Attribute MaterialBasic::attribute_at_index(const unsigned int attrib_idx) const {
    ...
    EDK::Type MaterialBasic::attribute_type_at_index(const unsigned int attrib_idx) const {
